...
const getMasterKeyFromSeed = (seed) => {
  const h = import_hmac.hmac.create(import_utils.sha512, ED25519_CURVE);
  const I = h.update((0, import_utils.fromHEX)(seed)).digest();
...
const CKDPriv = ({ key, chainCode }, index) => {
  ...
  const I = import_hmac.hmac.create(import_utils.sha512, chainCode).update(data).digest();
  const IL = I.slice(0, 32);
  const IR = I.slice(32);
  return {
    key: IL,
